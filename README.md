...
- `NOCWALL_STRIPE_PRICE_ID` (required only when using server-created Checkout Sessions)
- `NOCWALL_STRIPE_PAYMENT_LINK` (optional single Stripe payment link)
- `NOCWALL_STRIPE_PAYMENT_LINKS` (optional tier map; JSON or `tier=url` list)
- `NOCWALL_BILLING_DEFAULT_TIER` (default selected tier key, e.g. `starter_29`)
