...
| Meraki v1 | `POST /sources/meraki/poll`, `GET /sources/meraki/status` | Yes (`Account Settings` -> `Add NMS Source`) | `MERAKI_URL`, `MERAKI_TOKEN`, `MERAKI_DEVICES_PATH`, `MERAKI_AUTH_SCHEME`, `MERAKI_POLL_INTERVAL_SEC`, `MERAKI_POLL_RETRIES` | `bearer` | Yes (`demo=true` or missing creds) | Supported |
