...
run_cmd "curl -sS -L -c '$COOKIE_FILE' -b '$COOKIE_FILE' -X POST '$BASE_URL/?action=register' -d 'username=$NOCWALL_USER&password=$NOCWALL_PASS&password_confirm=$NOCWALL_PASS' >/dev/null"
run_cmd "curl -sS -L -c '$COOKIE_FILE' -b '$COOKIE_FILE' -X POST '$BASE_URL/?action=login' -d 'username=$NOCWALL_USER&password=$NOCWALL_PASS' >/dev/null"
...
demo_on="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" -X POST "$BASE_URL/?ajax=demo_mode_set" -d "enabled=1")"
...
prefs_save="$(curl -sS -c "$COOKIE_FILE" -b "$COOKIE_FILE" -X POST "$BASE_URL/?ajax=prefs_save" \
